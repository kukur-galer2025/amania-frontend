...
export default function MyEventDetailClient({ slug }: { slug: string }) {
 ...
 <p className="text-xs md:text-sm font-bold tracking-widest uppercase text-slate-400 dark:text-slate-400 text-center px-4">Mempersiapkan Ruang Kelas...</p>
 </div>
 );
 }
 ...
 <div className="max-w-md w-full bg-white dark:bg-[#111827] p-8 md:p-10 rounded-[2rem] shadow-sm dark:shadow-black/10 border border-slate-200 dark:border-slate-700/50 border-dashed">
 <AlertCircle size={48} className="mx-auto text-rose-500 mb-4 opacity-50 shrink-0"/>
 <h1 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-2">Akses Ditolak</h1>
 <p className="text-slate-500 dark:text-slate-400 mb-6 md:mb-8 text-xs md:text-sm leading-relaxed">Maaf, program ini tidak dapat diakses atau Anda belum terdaftar.</p>
 <Link href="/my-events" className="inline-flex px-5 py-2.5 bg-slate-900 text-white rounded-xl font-semibold text-xs md:text-sm hover:bg-indigo-600 shadow-sm dark:shadow-black/10 shrink-0">
 ...
 <ChevronRight size={14} className="text-slate-300 dark:text-slate-500 shrink-0 hidden sm:block"/>
 <span className="text-slate-900 dark:text-white font-bold truncate w-full ml-1 sm:ml-0">{event.title}</span>
 </div>
 </div>
 </nav>
 ...
 <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-black/10 p-4 sm:p-6 md:p-8 overflow-hidden w-full min-w-0">
 
 {/* AKSES PRIVAT (LINK ZOOM & REKAMAN) */}
 {(event.join_link || event.join_instructions || event.recording_link) && (
 <div className="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 p-5 md:p-6 rounded-2xl md:rounded-[1.5rem] shadow-sm dark:shadow-black/10 mb-6 md:mb-8 flex flex-col items-start w-full gap-4">
 
 {/* 1. Baris Akses Live Pertemuan */}
 {(event.join_link || event.join_instructions) && (
 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
 <div className="flex-1 min-w-0">
 <h3 className="text-sm md:text-base font-black text-indigo-900 dark:text-indigo-300 flex items-center gap-2 mb-1.5">
 <LinkIcon size={18} className="text-indigo-600 dark:text-indigo-400"/> Akses Pertemuan Live
 </h3>
 {event.join_instructions && (
 <p className="text-[11px] md:text-xs font-medium text-indigo-700 dark:text-indigo-400/80 leading-relaxed break-words w-full">
 {event.join_instructions}
 </p>
 )}
 </div>
 ...
 className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs md:text-sm rounded-xl shadow-lg dark:shadow-black/20 shadow-indigo-600/20 transition-transform flex items-center justify-center gap-2 shrink-0 active:scale-95"
 >
 ...
 <>
 {(event.join_link || event.join_instructions) && <div className="w-full border-t border-indigo-100 dark:border-indigo-500/20 my-1"/>}
 ...
 target="_blank"
 rel="noopener noreferrer"
 className="w-full sm:w-auto px-6 py-3 bg-white dark:bg-[#111827] border border-rose-200 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:bg-rose-500/10 dark:hover:bg-rose-950 hover:border-rose-300 font-bold text-xs md:text-sm rounded-xl shadow-sm dark:shadow-black/10 transition-transform flex items-center justify-center gap-2 shrink-0 active:scale-95"
 >
 ...
 {/* TABS BUTTONS */}
 <div className="flex p-1.5 bg-slate-100 dark:bg-slate-700/50 rounded-xl mb-6 md:mb-8 w-full min-w-0 overflow-x-auto custom-scrollbar">
 <button onClick={() => setActiveTab('curriculum')} className={`flex-1 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-transform rounded-lg truncate px-2 shrink-0 ${activeTab === 'curriculum' ? 'bg-white dark:bg-[#111827] text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-black/10 border border-slate-200 dark:border-slate-700/50' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white dark:text-white'}`}>
 Materi & Kurikulum
 </button>
 <button onClick={() => setActiveTab('desc')} className={`flex-1 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-transform rounded-lg truncate px-2 shrink-0 ${activeTab === 'desc' ? 'bg-white dark:bg-[#111827] text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-black/10 border border-slate-200 dark:border-slate-700/50' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white dark:text-white'}`}>
 Informasi Program
 </button>
 </div>
 ...
 <div className="py-12 md:py-16 bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 border-dashed rounded-2xl text-center px-4 w-full min-w-0">
 <BookOpen size={36} className="mx-auto text-slate-300 dark:text-slate-500 mb-3 md:mb-4 shrink-0"/>
 <h4 className="text-slate-900 dark:text-white font-bold text-sm md:text-base mb-1 md:mb-2">Materi Belum Tersedia</h4>
 <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm max-w-xs md:max-w-sm mx-auto leading-relaxed">Modul tambahan akan dipublikasikan oleh instruktur mendekati jadwal pelaksanaan atau sesudah acara.</p>
 </div>
 ) : (
 <>
 <div className="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 p-4 md:p-5 rounded-2xl mb-5 md:mb-8 w-full min-w-0 flex items-start gap-3">
 ...
 <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5 p-4 sm:p-5 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-black/10 rounded-2xl hover:border-indigo-300 hover:shadow-md dark:shadow-black/15 transition-transform group min-w-0 w-full relative overflow-hidden">
 
 <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center shrink-0 ${mat.access_tier === 'premium' ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-500' : mat.type === 'video' ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-500' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500'}`}>
 ...
 <h4 className="text-[13px] md:text-sm font-bold text-slate-900 dark:text-white leading-snug break-words group-hover:text-indigo-600 dark:text-indigo-400 line-clamp-2 w-full">
 {mat.title}
 </h4>
 </div>

 <div className="pt-2 sm:pt-0 sm:shrink-0 w-full sm:w-auto border-t sm:border-t-0 border-slate-100 dark:border-slate-700/50 mt-2 sm:mt-0">
 {mat.access_tier === 'premium' && event.user_registration?.tier !== 'premium' ? (
 <button className="w-full sm:w-auto flex items-center justify-center sm:justify-start gap-1.5 text-xs font-bold text-slate-400 dark:text-slate-400 bg-slate-50 dark:bg-[#111827] px-4 py-2.5 rounded-xl cursor-not-allowed border border-slate-200 dark:border-slate-700/50">
 <Lock size={14} className="shrink-0"/> Terkunci
 </button>
 ) : mat.type === 'video' && mat.link ? (
 <a href={mat.link} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center sm:justify-start gap-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-600 hover:text-white px-4 py-2.5 rounded-xl border border-indigo-100 dark:border-indigo-500/20 hover:border-transparent shadow-sm dark:shadow-black/10">
 ...
 className="w-full sm:w-auto flex items-center justify-center sm:justify-start gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-600 hover:text-white px-4 py-2.5 rounded-xl border border-emerald-100 dark:border-emerald-500/20 hover:border-transparent shadow-sm dark:shadow-black/10 disabled:opacity-50"
 >
 ...
 <div className="mt-8 md:mt-12 pt-6 md:pt-8 border-t border-slate-200 dark:border-slate-700/50 w-full min-w-0">
 ...
 <div key={spk.id} className="flex items-start gap-3 md:gap-4 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-[#111827] hover:bg-white dark:hover:bg-slate-700 dark:bg-[#111827] hover:shadow-md dark:shadow-black/15 transition-transform group min-w-0 w-full">
 <div className="w-12 h-12 md:w-14 md:h-14 rounded-full overflow-hidden border-2 border-slate-200/50 dark:border-slate-700/300 shadow-sm dark:shadow-black/10 shrink-0 bg-slate-200">
 ...
 <h4 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:text-indigo-400 w-full">{spk.name}</h4>
 ...
 <p className="text-sm sm:text-base font-black text-amber-950 uppercase tracking-wide drop-shadow-sm dark:shadow-black/10">
 {timeLeft}
 </p>
 </div>
 </div>
 ...
 {userTier && (
 <div className="mb-6 w-full flex items-center justify-between p-3.5 sm:p-4 rounded-2xl border bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-md dark:shadow-black/15">
 <div className="flex items-center gap-3">
 ...
 <h3 className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-widest mb-3 pb-2 border-b border-slate-100 dark:border-slate-700/50 flex items-center gap-1.5">
 <ShieldCheck size={12} className="shrink-0"/> Penyelenggara Resmi
 </h3>
 <div className="flex items-center gap-3 w-full min-w-0">
 <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 flex items-center justify-center shrink-0 overflow-hidden shadow-inner p-1">
 {organizerAvatar ? (
 ...
 <p className="text-[13px] md:text-sm font-bold text-slate-900 dark:text-white break-words w-full flex items-center gap-1.5">
 {organizerName} {isSuperadmin && <CheckCircle2 size={12} className="text-blue-500 shrink-0 fill-blue-50"/>}
 </p>
 <p className="text-[8px] md:text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-0.5 truncate w-full">
 ...
 <div className="space-y-4 mb-6 bg-slate-50 dark:bg-[#111827] p-4 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-700/50 w-full min-w-0">
 <div className="flex items-center justify-between text-[11px] md:text-xs min-w-0 w-full">
 <span className="text-slate-500 dark:text-slate-400 font-semibold flex items-center gap-1.5 shrink-0"><Users size={14} className="text-indigo-400 shrink-0"/> Kuota Kelas</span>
 ...
 className="w-full py-2.5 md:py-3 rounded-xl text-[11px] md:text-xs font-bold bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 text-amber-600 flex justify-center items-center gap-1.5 border border-amber-200 mb-3 shadow-sm dark:shadow-black/10"
 >
 ...
 disabled={isDownloading}
 className="w-full py-2.5 md:py-3 rounded-xl text-[11px] md:text-xs font-bold bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-indigo-600 dark:text-indigo-400 flex justify-center items-center gap-1.5 border border-indigo-100 dark:border-indigo-500/20 mb-3 shadow-sm dark:shadow-black/10 disabled:opacity-50"
 >
 ...
 <button onClick={handleShare} disabled={isSharing} className="w-full py-2.5 md:py-3 rounded-xl text-[11px] md:text-xs font-bold bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white dark:text-white hover:border-slate-300 dark:border-slate-600 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex justify-center items-center gap-1.5 shadow-sm dark:shadow-black/10 disabled:opacity-50">
