...
export default function ArticlesClient() {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#111827] font-sans selection:bg-blue-100 selection:text-blue-900 pb-16 md:pb-20 overflow-x-hidden">

            {/* 🔥 PERBAIKAN Z-INDEX DI SINI AGAR TIDAK MENEMBUS SIDEBAR 🔥 */}
            <div className="border-b border-slate-200 dark:border-slate-700/50 sticky top-0 z-20 bg-white dark:bg-slate-800/95 backdrop-blur-sm">
                <div className="max-w-6xl mx-auto px-4 lg:px-8 py-4 md:py-6 w-full">
                    <Link href="/beranda" className="inline-flex items-center gap-2 text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest hover:text-blue-600 mb-2 md:mb-3">
                ...
                <div className="max-w-6xl mx-auto px-4 lg:px-8 w-full">
                    <div className="flex items-center gap-4 md:gap-6 overflow-x-auto no-scrollbar py-2.5 md:py-3 border-t border-slate-100 dark:border-slate-700/50 w-full">
                        {categories.map((cat: any) => (
                            <button
                                key={cat}
                                onClick={() => setSelectedCategory(cat)}
                                className={`whitespace-nowrap text-xs md:text-sm font-bold uppercase tracking-wider shrink-0 ${selectedCategory === cat
                                    ? 'text-red-600 border-b-2 border-red-600 pb-1'
                                    : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white dark:text-white pb-1'
                                    }`}
                            >
                                {cat}
                            </button>
                ...
                <main className="w-full lg:w-[70%] min-w-0">
                    ...
                    <div className="lg:hidden mb-6 relative">
                        <input
                            ...
                            className="w-full bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 py-3 pl-10 pr-4 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-transform"
                        />
                    ...
                    {loading ? (
                        ...
                        <div className="py-16 md:py-20 text-center border-y border-slate-200 dark:border-slate-700/50 px-4">
                            <Search size={40} className="md:w-12 md:h-12 mx-auto text-slate-300 dark:text-slate-500 mb-3 md:mb-4" />
                            <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-1 md:mb-2">Berita Tidak Ditemukan</h3>
                            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400">Tidak ada artikel yang cocok dengan kata kunci"{searchQuery}".</p>
                        </div>
                    ) : (
                        <>
                            {/* HEADLINE (Hanya tampil di Halaman 1 jika ada data) */}
                            {currentPage === 1 && currentArticles.length > 0 && (
                                <div className="mb-8 md:mb-12 border-b border-slate-200 dark:border-slate-700/50 pb-8 md:pb-10 w-full min-w-0">
                                    ...
                                    <Link href={`/articles/${currentArticles[0].slug}`} className="group block w-full">
                                        <div className="w-full aspect-video md:aspect-[16/9] bg-slate-100 dark:bg-slate-700/50 mb-4 md:mb-5 relative overflow-hidden rounded-xl md:rounded-none shrink-0">
                                            ...
                                            <div className="absolute top-3 left-3 md:top-4 md:left-4 bg-red-600 text-white text-[9px] md:text-[10px] font-black uppercase tracking-widest px-2.5 py-1 md:px-3 rounded-md md:rounded-none shadow-md dark:shadow-black/15">
                                                {currentArticles[0].category?.name || 'Utama'}
                                            </div>
                                        </div>

                                        <div className="flex flex-wrap items-center gap-2 md:gap-3 text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 md:mb-3 uppercase tracking-wider px-1 md:px-0">
                                        ...
                                        <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 dark:text-white leading-tight mb-2 md:mb-4 group-hover:text-blue-600 px-1 md:px-0 break-words w-full">
                                            {currentArticles[0].title}
                                        </h2>

                                        <p className="text-sm md:text-base text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-3 md:line-clamp-none px-1 md:px-0 break-words w-full">
                            ...
                            <div className="space-y-0 w-full min-w-0">
                                <h3 className="text-base md:text-lg font-black uppercase border-l-4 border-red-600 pl-2.5 md:pl-3 mb-4 md:mb-6 text-slate-900 dark:text-white">
                                ...
                                <AnimatePresence mode="popLayout">
                                    {currentArticles.slice(currentPage === 1 ? 1 : 0).map((article: any, index: number) => (
                                        ...
                                        >
                                            {/* 🔥 PERUBAHAN: TAUTAN MENGARAH KE DYNAMIC ROUTE [slug] 🔥 */}
                                            <Link href={`/articles/${article.slug}`} className="group flex flex-col sm:flex-row gap-4 md:gap-5 py-5 md:py-6 border-b border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/50 px-1 md:px-2 rounded-xl md:rounded-none min-w-0">
                                                ...
                                                <div className="flex flex-col flex-1 py-1 min-w-0 w-full overflow-hidden">
                                                    ...
                                                    <h4 className="text-base sm:text-lg md:text-xl font-bold text-slate-900 dark:text-white leading-snug md:leading-tight mb-2 md:mb-3 group-hover:text-blue-600 line-clamp-2 md:line-clamp-3 break-words w-full">
                                                        {article.title}
                                                    </h4>

                                                    <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-3 md:mb-4 leading-relaxed break-words w-full">
                                                        {getSnippet(article.content)}
                                                    </p>

                                                    <div className="mt-auto flex items-center flex-wrap gap-2 md:gap-3 text-[9px] md:text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider shrink-0">
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-center gap-1.5 md:gap-2 mt-8 md:mt-12 border-t border-slate-200 dark:border-slate-700/50 pt-6 md:pt-8 overflow-x-auto pb-4 w-full">
                                    <button
                                        onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1}
                                        className="w-8 h-8 md:w-10 md:h-10 shrink-0 flex items-center justify-center border border-slate-300 dark:border-slate-600 text-slate-500 dark:text-slate-400 rounded-md md:rounded-none hover:border-slate-900 hover:text-slate-900 dark:hover:text-white dark:text-white disabled:opacity-30"
                                    >
                                    ...
                                    {[...Array(totalPages)].map((_, i) => (
                                        <button
                                            ...
                                            className={`w-8 h-8 md:w-10 md:h-10 shrink-0 flex items-center justify-center text-xs md:text-sm font-bold rounded-md md:rounded-none ${currentPage === i + 1
                                                ? 'bg-slate-900 text-white border border-slate-900'
                                                : 'bg-white dark:bg-[#111827] border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:border-slate-900 hover:text-slate-900 dark:hover:text-white dark:text-white'
                                                }`}
                                        >
                                            {i + 1}
                                        </button>
                ...
                <aside className="w-full lg:w-[30%] border-t border-slate-200 dark:border-slate-700/50 lg:border-t-0 pt-8 lg:pt-0 min-w-0">
                    <div className="lg:sticky lg:top-24 w-full">
                        ...
                        <div className="hidden lg:block mb-8 md:mb-10">
                            <h3 className="text-sm font-black uppercase border-b-2 border-slate-900 dark:border-slate-600 pb-2 mb-4 text-slate-900 dark:text-white flex items-center gap-2">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="w-full bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 py-3 pl-4 pr-10 text-sm focus:outline-none focus:border-blue-600"
                                />
                                <Search size={18} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-400" />
                            </div>
                        </div>

                        {/* WIDGET TERPOPULER */}
                        <div className="mb-10 w-full min-w-0">
                            <h3 className="text-base md:text-lg font-black text-slate-900 dark:text-white border-b-2 border-slate-900 dark:border-slate-600 pb-2 mb-5 md:mb-6 inline-block w-full flex items-center gap-2">
                                <TrendingUp size={18} className="text-red-600 shrink-0" /> Paling Populer
                            </h3>

                            <div className="space-y-0 w-full">
                                {popularArticles.map((article: any, idx: number) => (
                                    // 🔥 PERUBAHAN: TAUTAN MENGARAH KE DYNAMIC ROUTE [slug] 🔥
                                    <Link href={`/articles/${article.slug}`} key={idx} className="group flex gap-3 md:gap-4 py-3 md:py-4 border-b border-slate-100 dark:border-slate-700/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/50 px-2 rounded-lg w-full min-w-0">
                                        <div className="text-2xl md:text-3xl font-black text-slate-200 dark:text-slate-600 group-hover:text-red-600 mt-0.5 md:mt-0 shrink-0">
                                            0{idx + 1}
                                        </div>
                                        <div className="flex-1 min-w-0 overflow-hidden w-full">
                                            <span className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-widest block mb-1 truncate w-full">
                                                {article.category?.name || 'Umum'}
                                            </span>
                                            <h4 className="text-xs md:text-sm font-bold text-slate-800 dark:text-slate-200 leading-snug group-hover:text-blue-600 line-clamp-2 md:line-clamp-3 break-words w-full">
                                                {article.title}
                                            </h4>
                                        </div>
                                    </Link>
                                ...
                                {popularArticles.length === 0 && !loading && (
                                    <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-4">Belum ada berita terpopuler.</p>
                                )}
                            </div>
                        </div>
