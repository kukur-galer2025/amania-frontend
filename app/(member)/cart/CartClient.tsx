...
export default function CartClient() {
  ...
  const totals = useMemo(() => {
    ...
    selectedCartObjects.forEach(item => {
      const itemOriginal = item.product.original_price || (item.product.price * 10);
  ...
  if (loading) return (
    <div className="min-h-[60vh] flex flex-col justify-center items-center gap-4 relative z-10">
      ...
      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest animate-pulse">Sinkronisasi Keranjang...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-14 animate-in fade-in duration-700 relative z-10">
      ...
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-amber-500/10 rounded-full blur-[100px] -z-10 pointer-events-none mix-blend-multiply"></div>
      <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-orange-500/10 rounded-full blur-[100px] -z-10 pointer-events-none mix-blend-multiply"></div>

      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 border-b border-slate-200 pb-8">
        <div className="flex items-center gap-4">
          <div className="p-4 bg-gradient-to-br from-orange-500 to-amber-600 text-white rounded-2xl shadow-lg shadow-orange-500/20">
            <ShoppingCart size={32} strokeWidth={2.5} />
          </div>
          <div>
            <h1 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tight">Keranjang Belanja</h1>
            <p className="text-sm md:text-base font-bold text-slate-500 mt-1 flex items-center gap-2">
              <ShoppingBag size={16} className="text-amber-600" /> {cartItems.length} Aset Digital Tersimpan
            </p>
          </div>
        </div>
        <Link href="/e-products" className="text-xs md:text-sm font-bold text-amber-700 hover:text-amber-800 flex items-center gap-2 bg-amber-50 border border-amber-100 px-4 py-2 rounded-xl transition-all w-fit shadow-sm">
          <Store size={16} /> Lanjut Belanja
        </Link>
      </div>

      {cartItems.length === 0 ? (
        <div className="bg-white/50 backdrop-blur-xl border-2 border-dashed border-slate-300 hover:border-amber-300 transition-colors rounded-[2.5rem] p-12 md:p-24 flex flex-col items-center text-center shadow-sm group">
          <div className="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center mb-6 shadow-inner group-hover:-translate-y-2 transition-transform duration-500 border border-slate-100">
            <ShoppingCart size={40} className="text-amber-400" strokeWidth={2} />
          </div>
          <h2 className="text-2xl font-black text-slate-900 mb-3">Wah, Keranjangmu Kosong!</h2>
          <p className="text-slate-500 mb-10 max-w-md leading-relaxed font-medium text-sm md:text-base">
            Sepertinya Anda belum memilih materi premium. Yuk, jelajahi pustaka digital Amania dan tingkatkan keahlian Anda sekarang.
          </p>
          <Link href="/e-products" className="px-10 py-4 bg-gradient-to-r from-orange-600 to-amber-700 hover:from-orange-500 hover:to-amber-600 text-white rounded-2xl font-black transition-all shadow-lg shadow-amber-900/20 active:scale-95 flex items-center gap-3">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 items-start">
          ...
          <div className="flex-1 w-full flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between p-4 bg-white rounded-[1.5rem] border border-slate-100 shadow-sm">
              <button onClick={handleToggleSelectAll} className="flex items-center gap-3 group px-2">
                <div className={`w-5 h-5 md:w-6 md:h-6 rounded-[8px] border-2 flex items-center justify-center transition-all ${selectedItems.length === cartItems.length && cartItems.length > 0 ? 'bg-amber-500 border-amber-500 shadow-md shadow-amber-500/20' : 'border-slate-300 group-hover:border-amber-400 bg-slate-50'}`}>
                  {selectedItems.length === cartItems.length && cartItems.length > 0 && <Check size={16} className="text-white" strokeWidth={3} />}
                </div>
                <span className="text-sm md:text-base font-black text-slate-700 select-none">Pilih Semua ({cartItems.length})</span>
              </button>
            </div>
            ...
            <div className="space-y-4">
              {cartItems.map((item) => {
                 ...
                 return (
                  <div key={item.id} className={`group flex flex-row items-center gap-3 md:gap-4 p-3 md:p-5 bg-white rounded-[1.5rem] md:rounded-[2rem] border shadow-sm transition-all duration-300 relative overflow-hidden ${isSelected ? 'border-amber-300 shadow-[0_15px_40px_-10px_rgba(245,158,11,0.12)]' : 'border-slate-100 hover:border-amber-200'}`}>
                    ...
                    <button onClick={() => handleToggleSelect(item.id)} className="shrink-0 pl-1 md:pl-2 relative z-10 flex items-center justify-center">
                      <div className={`w-5 h-5 md:w-6 md:h-6 rounded-[8px] border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-amber-500 border-amber-500 shadow-md shadow-amber-500/20' : 'border-slate-300 group-hover:border-amber-400 bg-slate-50'}`}>
                        {isSelected && <Check size={16} className="text-white" strokeWidth={3} />}
                      </div>
                    </button>

                    <div className="w-[70px] sm:w-[110px] md:w-[130px] shrink-0 rounded-2xl overflow-hidden shadow-md border border-slate-100 aspect-[3/4] relative z-10 bg-slate-900 cursor-pointer" onClick={() => router.push(`/e-products/${item.product.slug}`)}>
                      {item.product.cover_image ? (
                         ...
                         <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 text-slate-300"><FileText size={24} /></div>
                      )}
                    ...
                    <div className="flex-1 flex flex-col min-w-0 py-1 relative z-10 h-full justify-between">
                      <div className="flex justify-between items-start gap-2 md:gap-4">
                        <div className="min-w-0 cursor-pointer pr-1 md:pr-2" onClick={() => router.push(`/e-products/${item.product.slug}`)}>
                          <h3 className="text-sm md:text-lg font-black text-slate-900 line-clamp-2 leading-snug group-hover:text-orange-700 transition-colors mb-2">{item.product.title}</h3>
                          <div className="flex flex-wrap items-center gap-1.5 md:gap-2 mt-1">
                            <span className="px-2 py-0.5 bg-amber-50 text-amber-700 text-[9px] sm:text-[10px] font-black uppercase tracking-wider rounded-md border border-amber-100 flex items-center gap-1">
                              <Layers size={10} /> Digital Asset
                            </span>
                            <span className="hidden sm:inline text-[9px] sm:text-[10px] font-bold text-slate-400">• Akses Selamanya</span>
                          </div>
                        </div>
                        <button onClick={() => handleRemoveItem(item.id)} className="p-2 text-rose-400 hover:text-white hover:bg-rose-500 rounded-full transition-all shrink-0 border border-transparent hover:border-rose-600 shadow-sm hover:shadow-rose-500/20 mt-[-5px] md:mt-0">
                      ...
                      <div className="mt-auto pt-3 md:pt-4 flex flex-col xl:flex-row xl:items-end justify-between gap-1">
                        {item.product.price > 0 && (
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] sm:text-xs font-bold text-slate-400 line-through decoration-rose-500/40">
                              {formatRupiah(itemOriginal)}
                            </span>
                            <span className="text-[8px] sm:text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-100 px-1.5 py-0.5 rounded">
                              {itemDiscountPercent}% OFF
                            </span>
                          </div>
                        )}
                        <p className="text-base sm:text-lg md:text-2xl font-black text-slate-900 tracking-tight w-full">
          ...
          <div className="w-full lg:w-[400px] shrink-0 lg:sticky lg:top-24">
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-200 shadow-[0_20px_50px_rgba(0,0,0,0.04)] relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-amber-50 rounded-full -mr-16 -mt-16 opacity-50" />
              
              <h2 className="text-xl font-black text-slate-900 mb-8 flex items-center gap-2 relative z-10">
              ...
              <div className="space-y-3 mb-6 relative z-10 max-h-[250px] overflow-y-auto custom-scrollbar pr-2">
                {cartItems.filter(item => selectedItems.includes(item.id)).length === 0 ? (
                  <p className="text-sm font-medium text-slate-400 italic text-center py-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  ...
                  cartItems.filter(item => selectedItems.includes(item.id)).map((item) => (
                    <div key={item.id} className="flex justify-between items-start gap-4 text-sm">
                      <span className="text-slate-600 font-medium line-clamp-2 leading-snug">{item.product.title}</span>
                      <span className="font-bold text-slate-800 shrink-0">{formatRupiah(item.product.price)}</span>
                    </div>
                  ))
                )}
              </div>

              <div className="border-t border-slate-100 my-5 relative z-10"></div>

              <div className="space-y-4 mb-8 relative z-10">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500 font-bold">Total Harga ({totals.count} barang)</span>
                  <span className="font-bold text-slate-400 line-through">{formatRupiah(totals.totalOriginal)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500 font-bold">Potongan Diskon</span>
                  <span className="font-black text-rose-500">- {formatRupiah(totals.totalDiscount)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500 font-bold">Biaya Layanan</span>
                  <span className="font-black text-emerald-500">Gratis</span>
                </div>
              </div>

              <div className="pt-6 border-t-2 border-dashed border-slate-100 mb-10 relative z-10">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-black text-slate-400 uppercase tracking-widest">Total Tagihan</span>
              ...
              <div className="space-y-3 relative z-10">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-4 md:py-5 rounded-2xl font-black text-sm md:text-base transition-all shadow-[0_8px_20px_rgba(5,150,105,0.2)] hover:shadow-[0_8px_20px_rgba(5,150,105,0.4)] active:scale-95 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none group overflow-hidden relative"
                >
                  ...
                  <FaMoneyIcon className={`shrink-0 w-[20px] h-[14px] ${selectedItems.length === 0 ? 'text-slate-400' : 'text-emerald-100'}`} />
                ...
                <div className="flex items-center justify-center gap-2 pt-4 opacity-50">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest truncate">Secure 256-bit SSL Payment</span>
                </div>
              </div>
            </div>

            <div className="mt-6 p-6 bg-amber-50/50 rounded-3xl border border-amber-100 flex gap-4">
              <Info className="text-amber-500 shrink-0" size={20} />
              <p className="text-[11px] md:text-xs text-amber-800/80 leading-relaxed font-medium">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]">
              <div className="p-5 md:p-6 border-b border-slate-100 flex justify-between items-center bg-amber-50/30">
                ...
                <button onClick={() => setIsPaymentModalOpen(false)} className="p-2 bg-white border border-slate-200 rounded-full text-slate-400 hover:text-rose-500 transition-colors"><X size={18}/></button>
              </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 md:gap-3">
                          {channels.map((c: any) => (
                            <button key={c.code} onClick={() => setSelectedChannel(c.code)} className={`flex items-center gap-3 p-3 rounded-[1rem] border-2 transition-all ${selectedChannel === c.code ? 'border-amber-600 bg-amber-50 shadow-md' : 'border-slate-200 hover:border-amber-200 bg-white'}`}>
                              <div className="w-10 h-6 shrink-0 bg-white rounded flex items-center justify-center p-0.5"><img src={c.icon_url} className="max-w-full max-h-full object-contain mix-blend-multiply" alt={c.name}/></div>
              ...
              <div className="p-5 md:p-6 bg-white border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="w-full sm:w-auto text-center sm:text-left"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Total Tagihan</p><p className="text-xl md:text-2xl font-black text-slate-900">{formatRupiah(totals.totalPrice)}</p></div>
                
                {/* 🔥 TOMBOL CHECKOUT MODAL TRIPAY 🔥 */}
                <button onClick={handleProcessCheckout} disabled={!selectedChannel || checkoutLoading} className="w-full sm:w-auto px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black shadow-lg shadow-emerald-600/20 active:scale-95 disabled:bg-slate-200 disabled:text-slate-400 transition-all flex items-center justify-center gap-2">
