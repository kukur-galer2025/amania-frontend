...
import { 
  ...
  User, Zap, Share2, Ticket, AlertCircle, Loader2, CheckCircle2,
  Gem, Image as ImageIcon, BookOpen, Video, FileText, Lock, Briefcase, 
  DownloadCloud, Sparkles, Info // 🔥 Perbaikan: Info dan Briefcase sudah di-import
} from 'lucide-react';
...
export default function EventDetailClient({ slug }: { slug: string }) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-5 w-full">
      <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1.2, ease: 'linear' }}>
        <Loader2 size={48} className="text-indigo-600" />
      </motion.div>
      <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-xs">Menyiapkan Ruang Kelas</p>
    </div>
  );

  if (!eventData) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center w-full px-4 bg-slate-50">
      ...
      <p className="text-slate-500 text-sm max-w-sm mb-4">Program yang Anda cari mungkin sudah dihapus atau tautan tidak valid.</p>
      <Link href="/events" className="px-8 py-3.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-indigo-600 transition-colors shadow-lg">
        Kembali ke Katalog
      </Link>
    </div>
  );
  ...
  const organizerAvatar = !isSuperadmin && eventData.organizer?.avatar ? `${STORAGE_URL}/${eventData.organizer.avatar}` : null;

  return (
    <div className="min-h-screen bg-slate-50/50 font-sans w-full overflow-x-hidden pb-32 lg:pb-16 selection:bg-indigo-200 selection:text-indigo-900">
      
      {/* HEADER BAR (STICKY) */}
      <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 h-16 flex items-center z-50 transition-all shadow-sm">
        <div className="max-w-7xl mx-auto w-full px-4 md:px-8 flex items-center justify-between">
          <Link href="/events" className="group flex items-center gap-2.5 text-slate-700 font-bold text-sm hover:text-indigo-600 transition-colors">
            <div className="p-1.5 bg-slate-100 rounded-full group-hover:bg-indigo-100 transition-all">
              <ArrowLeft size={16} />
            </div>
            <span className="hidden sm:inline">Katalog Program</span>
          </Link>
          <div className="flex gap-2">
            <button onClick={handleShare} className="flex items-center gap-2 px-4 py-2 bg-indigo-50 border border-indigo-100 text-indigo-600 rounded-full text-xs font-bold hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
      ...
      {/* HERO SECTION - MODERN PARALLAX-ISH */}
      <div className="relative w-full pt-16">
        <div className="max-w-7xl mx-auto px-0 sm:px-4 md:px-8 sm:pt-6">
          <div className="relative aspect-square sm:aspect-[21/9] w-full overflow-hidden sm:rounded-[2.5rem] shadow-2xl group bg-slate-900">
            {eventData.image ? (
              <img src={`${STORAGE_URL}/${eventData.image}`} alt="Banner" className="w-full h-full object-cover opacity-80 transition-transform duration-[2s] group-hover:scale-105" />
            ) : (
            ...
            <div className="absolute bottom-0 left-0 p-6 sm:p-10 md:p-12 w-full">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 max-w-4xl">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  ...
                  <span className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 shadow-lg ${isPast ? 'bg-rose-500/90 text-white' : 'bg-emerald-500/90 text-white'}`}>
      ...
      {/* CONTENT AREA */}
      <main className="max-w-7xl mx-auto px-4 md:px-8 mt-6 md:mt-10 grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 md:gap-10">
        ...
        <div className="space-y-8 order-2 lg:order-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-4 sm:p-5 rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col gap-1.5">
              <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-500 mb-1"><Calendar size={16}/></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tanggal Pelaksanaan</p>
              <p className="text-sm font-bold text-slate-900">{eventDate.toLocaleDateString('id-ID', {day:'numeric', month:'long', year:'numeric'})}</p>
            </div>
            <div className="bg-white p-4 sm:p-5 rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col gap-1.5">
              <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500 mb-1"><Clock size={16}/></div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Waktu Akses</p>
              <p className="text-sm font-bold text-slate-900">{eventDate.toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})} WIB</p>
            </div>
            <div className="bg-white p-4 sm:p-5 rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col gap-1.5 col-span-2 md:col-span-1">
              <div className="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center text-rose-500 mb-1"><MapPin size={16}/></div>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-5 sm:p-8">
            {/* TABS NAVIGATION */}
            <div className="flex gap-4 sm:gap-8 border-b border-slate-100 mb-8 overflow-x-auto custom-scrollbar">
              <button onClick={() => setActiveTab('desc')} className={`pb-4 text-sm font-black transition-all relative whitespace-nowrap px-1 ${activeTab === 'desc' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-700'}`}>
                Informasi Program
                {activeTab === 'desc' && <motion.div layoutId="tabLine" className="absolute bottom-0 left-0 right-0 h-1 bg-indigo-600 rounded-t-full" />}
              </button>
              <button onClick={() => setActiveTab('curriculum')} className={`pb-4 text-sm font-black transition-all relative whitespace-nowrap px-1 ${activeTab === 'curriculum' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-700'}`}>
                Materi / Kurikulum
                {activeTab === 'curriculum' && <motion.div layoutId="tabLine" className="absolute bottom-0 left-0 right-0 h-1 bg-indigo-600 rounded-t-full" />}
              </button>
            </div>
              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'desc' ? (
                  <motion.div key="desc" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}>
                    <div className="prose prose-slate max-w-none prose-headings:font-black prose-p:text-slate-600 prose-p:leading-relaxed break-words" dangerouslySetInnerHTML={{ __html: cleanHtml(eventData.description) }} />
                      ...
                      <div className="mt-10 pt-8 border-t border-slate-100">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {eventData.speakers.map((spk: any) => (
                            <div key={spk.id} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-200 transition-colors group">
                              <img src={`${STORAGE_URL}/${spk.photo}`} alt={spk.name} className="w-14 h-14 rounded-full object-cover shadow-sm group-hover:scale-105 transition-transform" />
                              <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-bold text-slate-900 truncate">{spk.name}</h4>
                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 truncate flex items-center gap-1 mt-1">
                                  <Briefcase size={12} className="text-indigo-400" /> {spk.role}
                                </p>
                              </div>
                            </div>
                          ))}
                ...
                /* CURRICULUM TAB */
                : (
                  <motion.div key="curriculum" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-4">
                    {(!eventData.materials || eventData.materials.length === 0) ? (
                      <div className="py-16 text-center border border-dashed border-slate-200 rounded-2xl bg-slate-50">
                        ...
                        <p className="text-sm text-slate-500 max-w-sm mx-auto">Syllabus lengkap akan segera dipublikasikan menjelang pelaksanaan program.</p>
                      </div>
                    ) : (
                      <>
                        <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl mb-6 w-full flex items-start gap-3">
                            <Info size={18} className="text-indigo-500 shrink-0 mt-0.5" />
                            <div>
                              <p className="text-xs md:text-sm font-bold text-indigo-900 mb-1">Daftar Kurikulum</p>
                              <p className="text-[11px] md:text-xs text-indigo-700 leading-relaxed">Berikut adalah materi yang akan dibahas. Materi dengan label <strong>VIP</strong> hanya bisa diakses secara penuh oleh peserta Premium setelah acara berlangsung.</p>
                            </div>
                        </div>

                        {eventData.materials.map((mat: any, i: number) => (
                          <div key={i} className="flex items-center gap-4 p-5 bg-white border border-slate-200 rounded-3xl hover:border-indigo-600 transition-all group">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">Modul {i+1}</span>
                                {mat.access_tier === 'premium' ? (
                                  <span className="text-[8px] font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded flex items-center gap-1"><Gem size={8}/> VIP ONLY</span>
                                ) : (
                                  <span className="text-[8px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded flex items-center gap-1"><CheckCircle2 size={8}/> BASIC</span>
                                )}
                              </div>
        ...
        {/* RIGHT COLUMN - CHECKOUT SIDEBAR (STICKY ON DESKTOP) */}
        <div className="order-1 lg:order-2 w-full lg:sticky lg:top-24">
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl shadow-slate-200/50 relative overflow-hidden">
            ...
            <div className="space-y-4 mb-8">
              ...
              <button 
                ...
                className={`w-full p-4 sm:p-5 rounded-2xl border-2 transition-all flex justify-between items-center text-left ${selectedTier === 'basic' ? 'border-indigo-600 bg-indigo-50/50 shadow-md shadow-indigo-500/10' : 'border-slate-100 bg-white hover:border-slate-300 opacity-70'}`}
              >
                <div>
              ...
              {eventData.premium_price > 0 && (
                <button 
                  ...
                  className={`w-full p-4 sm:p-5 rounded-2xl border-2 transition-all flex justify-between items-center text-left ${selectedTier === 'premium' ? 'border-amber-500 bg-amber-50/50 shadow-md shadow-amber-500/10' : 'border-slate-100 bg-white hover:border-slate-300 opacity-70'}`}
                >
                  <div>
            ...
            <div className="hidden lg:block space-y-3 mb-8">
              ...
              {eventData.image && (
                <button onClick={handleDownloadBanner} className="w-full py-3.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-xs hover:border-indigo-500 hover:text-indigo-600 transition-colors flex items-center justify-center gap-2">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3 mb-6">
              <div className="flex items-center justify-between text-xs min-w-0">
                ...
                <span className="font-bold text-slate-900 truncate bg-white px-2 py-1 rounded-md border border-slate-200">{eventData.quota === 0 ? 'Habis' : `${eventData.quota} Kursi`}</span>
              </div>
              <div className="flex items-center justify-between text-xs min-w-0">
                <span className="text-slate-500 font-semibold shrink-0">E-Certificate</span>
                <span className="font-bold text-emerald-600 truncate bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200">{eventData.certificate_tier === 'none' ? 'Tidak Ada' : 'Tersedia'}</span>
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-slate-100 flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200 shrink-0">
                {organizerAvatar ? <img src={organizerAvatar} className="w-full h-full object-cover" alt="org"/> : <User size={20} className="text-slate-400" />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-slate-900 flex items-center gap-1 truncate">
                  {organizerName} {isSuperadmin && <ShieldCheck size={14} className="text-emerald-500 shrink-0" />}
                </p>
              </div>
            </div>
      ...
      {/* 🔥 MOBILE STICKY BOTTOM BAR (Tampil di Layar Kecil) 🔥 */}
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 p-4 pb-safe z-50 lg:hidden shadow-[0_-10px_40px_rgba(0,0,0,0.08)] flex items-center justify-between gap-3">
        <div className="shrink-0 min-w-0 max-w-[40%]">
          ...
          <p className={`text-lg sm:text-xl font-black tracking-tight leading-none truncate ${selectedTier === 'premium' ? 'text-amber-600' : isFree ? 'text-emerald-500' : 'text-slate-900'}`}>
            {selectedTier === 'premium' 
              ? `Rp ${eventData.premium_price.toLocaleString('id-ID')}` 
              : isFree ? 'GRATIS' : `Rp ${eventData.basic_price.toLocaleString('id-ID')}`
            }
        ...
        >
          <span className="truncate">{isPast ? 'Ditutup' : eventData.quota === 0 ? 'Penuh' : 'Lanjut Bayar'}</span>
        </button>

        {/* Tombol Download Poster Mini Khusus Mobile */}
        {eventData.image && (
           <button onClick={handleDownloadBanner} className="p-3.5 sm:p-4 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 transition-colors shrink-0 flex items-center justify-center">
             <DownloadCloud size={18} />
           </button>
        )}
      </div>

      {/* GLOBAL CSS FOR PROSE (HTML FROM DB) */}
      <style jsx global>{`
        .prose img { border-radius: 1.5rem; margin: 2rem 0; box-shadow: 0 10px 40px rgba(15,23,42,0.06); width: 100%; height: auto; }
        .prose iframe, .prose video { width: 100%; border-radius: 1rem; margin: 1.5rem 0; }
        .prose h2 { font-size: 1.5rem; margin-top: 2.5rem; margin-bottom: 1rem; color: #0f172a; font-weight: 900;}
        .prose h3 { font-size: 1.25rem; margin-top: 2rem; margin-bottom: 0.75rem; color: #0f172a; font-weight: 800;}
