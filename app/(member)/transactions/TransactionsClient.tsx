...
const InvoiceCountdown = ({ createdAt, expiredAt }: { createdAt: string, expiredAt?: number | string | null }) => {
 ...
 <span className="text-sm font-bold text-rose-600 dark:text-rose-400 truncate w-full">Waktu Habis</span>
 </div>
 );
 }
 ...
 return (
 <div className="flex flex-col min-w-0">
 <span className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1 mb-0.5">
...
export default function TransactionsClient() {
 ...
 const [courseTransactions, setCourseTransactions] = useState<any[]>([]);
 const [loading, setLoading] = useState(true);
 
 const [activeTab, setActiveTab] = useState<'event' | 'eproduct' | 'course'>('event');
 ...
 return (
 <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-200 rounded-md text-[10px] font-black uppercase tracking-widest shadow-sm dark:shadow-black/10 shrink-0">
 ...
 return (
 <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-50 dark:bg-rose-500/10 text-rose-700 border border-rose-200 rounded-md text-[10px] font-black uppercase tracking-widest shadow-sm dark:shadow-black/10 shrink-0">
 ...
 return (
 <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-700 border border-amber-200 rounded-md text-[10px] font-black uppercase tracking-widest shadow-sm dark:shadow-black/10 shrink-0">
 ...
 if (loading) {
 return (
 <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4 bg-[#F8FAFC] dark:bg-[#0B1120]">
 <Loader2 className="animate-spin text-indigo-600 dark:text-indigo-400" size={40} strokeWidth={2.5}/>
 <span className="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-widest uppercase animate-pulse">Memuat Transaksi...</span>
 </div>
 );
 }

 return (
 <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] pb-24 font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900 w-full min-w-0">
 
 {/* ════ HEADER SECTION ════ */}
 <div className="bg-white dark:bg-[#111827] border-b border-slate-200 dark:border-slate-700/50 sticky top-0 z-30 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] w-full min-w-0">
 ...
 <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-100/50 border border-indigo-200/50 rounded-full text-indigo-700 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest mb-4">
 ...
 {/* 🔥 TABS NAVIGATION 🔥 */}
 <div className="flex gap-8 border-b border-slate-200 dark:border-slate-700/50 overflow-x-auto hide-scroll-bar w-full">
 <button 
 onClick={() => setActiveTab('event')}
 className={`pb-4 text-sm font-black transition-transform relative whitespace-nowrap flex items-center gap-2 ${activeTab === 'event' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:text-slate-400'}`}
 >
 ...
 className={`pb-4 text-sm font-black transition-transform relative whitespace-nowrap flex items-center gap-2 ${activeTab === 'eproduct' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:text-slate-400'}`}
 >
 ...
 className={`pb-4 text-sm font-black transition-transform relative whitespace-nowrap flex items-center gap-2 ${activeTab === 'course' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:text-slate-400'}`}
 >
 <GraduationCap size={18} className={activeTab === 'course' ? 'text-indigo-500' : ''} /> Kursus Online
 {activeTab === 'course' && <motion.div layoutId="activeTabTrx" className="absolute bottom-0 left-0 right-0 h-1 bg-indigo-600 rounded-t-full shadow-[0_0_8px_rgba(79,70,229,0.5)]"/>}
 </button>
 </div>
 </div>
 </div>
 ...
 className="w-full bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-xl py-2.5 md:py-3 pl-10 pr-4 text-xs md:text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:focus:ring-indigo-500/20 transition-transform shadow-[0_2px_10px_rgba(0,0,0,0.02)]"
 />
 </div>

 <div className="flex p-1 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-xl overflow-x-auto hide-scroll-bar w-full md:w-auto shadow-[0_2px_10px_rgba(0,0,0,0.02)] shrink-0">
 {[
 { id: 'all', label: 'Semua' },
 ...
 className={`flex-1 md:flex-none px-4 md:px-5 py-2 text-[10px] md:text-xs font-bold uppercase tracking-wider rounded-lg transition-transform whitespace-nowrap ${
 filterStatus === tab.id 
 ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 shadow-sm dark:shadow-black/10 border border-indigo-100 dark:border-indigo-500/20' 
 : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 border border-transparent'
 }`}
 >
 {tab.label}
 </button>
 ...
 <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 border-dashed rounded-[2rem] p-12 md:p-20 flex flex-col items-center justify-center text-center shadow-sm dark:shadow-black/10 w-full">
 <div className="w-20 h-20 bg-slate-50 dark:bg-[#111827] border border-slate-100 dark:border-slate-700/50 text-slate-300 dark:text-slate-500 rounded-full flex items-center justify-center mb-6 shadow-sm dark:shadow-black/10">
 ...
 {searchQuery || filterStatus !== 'all' ? (
 <button onClick={() => { setSearchQuery(''); setFilterStatus('all'); }} className="px-6 py-3 bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 text-sm font-bold rounded-xl transition-transform">
 Reset Pencarian
 </button>
 ) : (
 <Link href="/events" className="px-6 py-3 bg-slate-900 hover:bg-indigo-600 text-white text-sm font-bold rounded-xl transition-transform shadow-md dark:shadow-black/15 flex items-center gap-2">
 ...
 <div key={`evt-${trx.id}`} className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 rounded-[1.5rem] p-4 md:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(79,70,229,0.1)] hover:border-indigo-200 dark:hover:border-indigo-700 transition-transform duration-300 group flex flex-col gap-4 w-full min-w-0">
 
 {/* BAGIAN ATAS: INFO & GAMBAR */}
 <div className="flex flex-col sm:flex-row gap-4 md:gap-6 w-full">
 <div className="w-full sm:w-48 md:w-56 shrink-0 rounded-[1rem] bg-slate-900 overflow-hidden border border-slate-200 dark:border-slate-700/50 relative shadow-sm dark:shadow-black/10 aspect-video flex items-center justify-center p-1.5 md:p-2">
 {trx.event?.image ? (
 <React.Fragment>
 ...
 <div className="flex-1 flex flex-col w-full min-w-0 justify-center">
 <div className="flex flex-wrap items-center gap-2 mb-2 md:mb-3 w-full min-w-0">
 {getStatusBadge(trx.status, trx.created_at)}
 <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 text-slate-600 dark:text-slate-400 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-widest truncate shrink-0">
 ...
 <h3 className="text-base md:text-xl font-black text-slate-900 dark:text-white group-hover:text-indigo-600 dark:text-indigo-400 line-clamp-2 leading-tight mb-2 w-full break-words">
 {trx.event?.title || <span className="italic text-slate-400 dark:text-slate-400">Event tidak tersedia</span>}
 </h3>
 ...
 <span className="text-[9px] md:text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1 mb-0.5">
 <CreditCard size={12} /> Total Pembayaran
 </span>
 <span className="text-lg md:text-2xl font-black text-slate-900 dark:text-white font-mono tracking-tight w-full truncate">
 ...
 <Link href="/e-products" className="px-6 py-3 bg-slate-900 hover:bg-indigo-600 text-white text-sm font-bold rounded-xl transition-transform shadow-md dark:shadow-black/15 flex items-center gap-2">
 ...
 <h3 className="text-sm sm:text-base md:text-xl font-black text-slate-900 dark:text-white group-hover:text-indigo-600 dark:text-indigo-400 line-clamp-2 sm:line-clamp-3 leading-tight mb-2 w-full break-words">
 {productNames || <span className="italic text-slate-400 dark:text-slate-400">Produk tidak tersedia</span>}
 </h3>

 {trx.payment_method && (
 <div className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 w-full min-w-0">
 Metode Bayar: <span className="text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-700/50 px-2 py-0.5 rounded-md uppercase tracking-wider text-[9px] md:text-[10px] border border-slate-200 dark:border-slate-700/50 shrink-0">{trx.payment_method}</span>
 </div>
 )}
 </div>
 </div>
 ...
 {/* ========================================================================= */}
 {/* TAB 3: TRANSAKSI KURSUS ONLINE (TRIPAY) */}
 {/* ========================================================================= */}
 {activeTab === 'course' && (
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full min-w-0">
 ...
 <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">
 {searchQuery || filterStatus !== 'all' ? 'Hasil Pencarian Kosong' : 'Belum Ada Transaksi Kursus'}
 </h3>
 <p className="text-slate-500 dark:text-slate-400 text-sm font-medium max-w-sm leading-relaxed mb-8">
 {searchQuery || filterStatus !== 'all' 
 ? 'Tidak ada transaksi yang cocok dengan kata kunci atau filter saat ini.' 
 : 'Anda belum membeli kursus online apapun. Temukan kursus menarik di katalog kami.'}
 </p>
 {searchQuery || filterStatus !== 'all' ? (
 <button onClick={() => { setSearchQuery(''); setFilterStatus('all'); }} className="px-6 py-3 bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 text-sm font-bold rounded-xl transition-transform">
 Reset Pencarian
 ...
 <Link href="/courses" className="px-6 py-3 bg-slate-900 hover:bg-emerald-600 text-white text-sm font-bold rounded-xl transition-transform shadow-md dark:shadow-black/15 flex items-center gap-2">
 Cari Kursus Sekarang <ArrowUpRight size={16} />
 </Link>
 )}
 </div>
 ) : (
 ...
 if (trx.expired_time) {
 isTimeExpired = new Date().getTime() > (Number(trx.expired_time) * 1000);
 } else if (trx.created_at) {
 const createdTime = parseSafeDate(trx.created_at).getTime();
 isTimeExpired = new Date().getTime() > (createdTime + 24 * 60 * 60 * 1000);
 }

 const isActuallyUnpaid = (s === 'unpaid' || s === 'pending') && !isTimeExpired;
 const courseData = trx.course;
 ...
 {getStatusBadge(trx.status, trx.created_at, trx.expired_time)}
 <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 text-slate-600 dark:text-slate-400 rounded-md text-[9px] md:text-[10px] font-black uppercase tracking-widest truncate shrink-0">
 <Receipt size={12}/> Ref: {trx.reference || `TRX-CRS-${trx.id}`}
 </span>
 <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-50 dark:bg-[#111827] border border-slate-200 dark:border-slate-700/50 text-slate-600 dark:text-slate-400 rounded-md text-[9px] md:text-[10px] font-black uppercase tracking-widest truncate shrink-0">
 <CalendarDays size={12}/> {formatDateTime(trx.created_at)}
 </span>
 ...
 <h3 className="text-sm sm:text-base md:text-xl font-black text-slate-900 dark:text-white group-hover:text-emerald-600 dark:text-emerald-400 line-clamp-2 leading-tight mb-2 w-full break-words">
 {courseData?.title || <span className="italic text-slate-400 dark:text-slate-400">Kursus tidak tersedia</span>}
 </h3>
 ...
 className="relative max-w-xl w-full flex flex-col items-center bg-white dark:bg-[#111827] p-3 rounded-[2rem] shadow-2xl dark:shadow-black/25 border border-slate-200 dark:border-slate-700/50 min-w-0"
 onClick={(e) => e.stopPropagation()} 
 >
 <div className="w-full flex justify-between items-center px-4 py-3 mb-2 border-b border-slate-100 dark:border-slate-700/50 min-w-0">
 <span className="text-[10px] md:text-xs font-black text-slate-800 dark:text-slate-200 flex items-center gap-2 uppercase tracking-widest truncate">
 <ImageIcon size={16} className="text-indigo-500 shrink-0"/> Bukti Pembayaran Manual
 </span>
 <button 
 onClick={() => setSelectedProof(null)}
 className="p-1.5 bg-slate-100 dark:bg-slate-700/50 hover:bg-rose-100 text-slate-500 dark:text-slate-400 hover:text-rose-500 rounded-xl shrink-0"
 >
 <X size={18} />
 </button>
 </div>
 ...
 className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 md:py-3 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-slate-800 shadow-lg dark:shadow-black/20 min-w-0"
 >
