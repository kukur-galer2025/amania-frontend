...
export default function EProductDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // 🔥 PASTIKAN KIRIM TOKEN AGAR BACKEND BISA CEK STATUS BELI DENGAN TYPE YANG BENAR 🔥
  ...
  return (
    <div className="min-h-screen bg-slate-50/80 font-sans pb-28 lg:pb-24 selection:bg-indigo-100 selection:text-indigo-900 w-full overflow-x-hidden flex flex-col">
      <style jsx global>{`
      ...
      {/* HEADER NAV */}
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm w-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 md:h-16 flex items-center">
          <Link href="/e-products" className="inline-flex items-center gap-2 text-slate-600 hover:text-indigo-600 font-bold text-xs md:text-sm transition-colors bg-slate-100 hover:bg-indigo-50 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-slate-200 hover:border-indigo-200">
            <ArrowLeft size={16} /> Kembali <span className="hidden sm:inline">ke Katalog</span>
          </Link>
        </div>
      </div>

      {/* 🔥 KEMBALIKAN BATASAN MAX-WIDTH DI SINI 🔥 */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 md:mt-12 w-full">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
          ...
          <div className="flex-1 w-full min-w-0 flex flex-col gap-8 md:gap-10">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} 
              className="w-full aspect-[4/3] sm:aspect-video md:aspect-[21/9] rounded-[1.5rem] md:rounded-[2rem] bg-slate-200 overflow-hidden relative shadow-sm border border-slate-200/60 group shrink-0">
              {product.cover_image ? (
            ...
            {/* INFO PRODUK */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="flex flex-col gap-5 md:gap-6 min-w-0">
              <h1 className="text-2xl sm:text-4xl md:text-5xl font-black text-slate-900 leading-[1.2] tracking-tight break-words w-full">
                {product.title}
              </h1>
              
              <div className="flex flex-wrap items-center gap-x-5 gap-y-4 pb-6 md:pb-8 border-b border-slate-200 w-full min-w-0">
                <div className="flex items-center gap-2.5 md:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs md:text-sm font-bold text-slate-900 truncate">{product.author?.name || 'Amania Official'}</p>
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} 
              className="mt-4 md:mt-8 bg-white border border-slate-200 rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-10 shadow-sm w-full">
              <h2 className="text-xl md:text-2xl font-black text-slate-900 mb-6 md:mb-8 flex items-center gap-2.5 md:gap-3">
                <MessageSquare className="text-indigo-500 w-5 h-5 md:w-6 md:h-6 shrink-0" /> Ulasan Pembeli
              </h2>

              {reviews.length === 0 ? (
                <div className="text-center py-10 md:py-12 bg-slate-50 rounded-2xl border border-slate-100 border-dashed px-4 w-full">
                ...
                <div className="grid gap-3 md:gap-4 mb-8 md:mb-10 w-full">
                  {reviews.map((r: any) => (
                    <div key={r.id} className="p-4 md:p-5 rounded-2xl bg-slate-50 border border-slate-100 flex gap-3 md:gap-4 w-full min-w-0">
                      ...
                      <div className="min-w-0 flex-1 w-full">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1.5 sm:gap-2 mb-2 w-full min-w-0">
                          <div className="min-w-0">
              ...
              <div className="w-full">
                <h3 className="text-base md:text-lg font-bold text-slate-900 mb-4 md:mb-6 flex items-center gap-2">
                  <Star className="text-amber-500 fill-amber-500 w-4 h-4 md:w-5 md:h-5 shrink-0"/> {submitted ? 'Ulasan Kamu' : 'Bagikan Pengalamanmu'}
                </h3>

                {!userData ? (
                  <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left w-full">
                    <div>
                    ...
                    <Link href="/login" className="w-full sm:w-auto px-6 py-3 bg-indigo-600 text-white text-xs md:text-sm font-bold rounded-xl hover:bg-indigo-700 transition-colors whitespace-nowrap text-center shrink-0">
                      Masuk ke Akun
                    </Link>
                  </div>
                ) 
                : !isOwned ? (
                  <div 
                    ...
                    className="bg-slate-50 border border-slate-200 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-10 flex flex-col items-center text-center cursor-pointer hover:bg-slate-100 transition-colors group relative overflow-hidden w-full"
                  >
                ...
                : submitted ? (
                  <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 w-full min-w-0">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-emerald-800 text-sm md:text-base mb-2 flex items-center gap-2"><CheckCircle2 size={16} className="shrink-0"/> Ulasan tersimpan!</p>
                      <StarRow rating={myRating} size={16} />
                      {myReview && <p className="text-xs md:text-sm text-emerald-700 mt-3 bg-white/50 p-3 rounded-lg break-words w-full">{myReview}</p>}
                    </div>
                  ...
                  <div className="space-y-4 md:space-y-5 w-full">
                    <div>
                      ...
                      <div className="flex gap-1 md:gap-2">
                        {[1,2,3,4,5].map((s) => (
                          <button key={s} onClick={() => setMyRating(s)} className="p-1 hover:scale-110 transition-transform focus:outline-none shrink-0">
                    ...
                    <button onClick={handleSubmitReview} disabled={submitting || myRating === 0} 
                      ...
                      {submitting ? <Loader2 size={16} className="animate-spin shrink-0" /> : <Send size={16} className="shrink-0" />} Kirim Ulasan
                    </button>
                  </div>
                )}
              </div>
          ...
          <div className="w-full lg:w-[380px] shrink-0 lg:sticky lg:top-24 mt-8 lg:mt-0">
            <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/30 overflow-hidden">
              <div className="p-5 sm:p-8">
                ...
                <div className="hidden lg:block w-full">
                  {isOwned ? (
                    <button onClick={handleAccessProduct} className="w-full py-4 rounded-xl font-black text-white bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 shadow-lg shadow-emerald-500/25 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2">
                      <DownloadCloud size={20} className="shrink-0" /> Buka / Akses Materi
                    </button>
                  ) : (
                    <button onClick={handleOpenPaymentModal} disabled={btnLoading} className="w-full py-4 rounded-xl font-black text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-lg shadow-indigo-500/25 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:translate-y-0">
                      {btnLoading ? <Loader2 size={20} className="animate-spin shrink-0" /> : <ShoppingCart size={20} className="shrink-0" />}
                ...
                <div className="flex items-center justify-center gap-2 mt-4 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                  <ShieldCheck size={14} className="text-emerald-500 shrink-0" /> 100% Aman & Terenkripsi
                </div>
              </div>

              <div className="bg-slate-50 border-t border-slate-100 p-5 sm:p-8 flex flex-col gap-4 md:gap-5 w-full">
                {BENEFITS.map((b) => (
                  <div key={b.label} className="flex gap-3 md:gap-4 w-full min-w-0">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 p-4 pb-safe z-50 lg:hidden shadow-[0_-10px_30px_rgba(15,23,42,0.06)] flex items-center justify-between gap-4 w-full">
        <div className="shrink-0">
        ...
        {isOwned ? (
          <button onClick={handleAccessProduct} className="flex-1 py-3.5 rounded-xl font-black text-white text-sm bg-gradient-to-r from-emerald-500 to-emerald-600 shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 active:scale-95 transition-transform min-w-0">
            <DownloadCloud size={18} className="shrink-0" /> <span className="truncate">Buka Materi</span>
          </button>
        ) : (
          <button onClick={handleOpenPaymentModal} disabled={btnLoading} className="flex-1 py-3.5 rounded-xl font-black text-white text-sm bg-gradient-to-r from-indigo-600 to-purple-600 shadow-lg shadow-indigo-500/25 flex items-center justify-center gap-2 disabled:opacity-70 active:scale-95 transition-transform min-w-0">
            {btnLoading ? <Loader2 size={18} className="animate-spin shrink-0" /> : <ShoppingCart size={18} className="shrink-0" />}
            <span className="truncate">{isFree ? 'Klaim Gratis' : 'Beli Sekarang'}</span>
          </button>
        )}
      </div>
      ...
      <AnimatePresence>
        {isLockedModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsLockedModalOpen(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm w-full h-full" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-[90%] sm:max-w-md bg-white rounded-[2rem] shadow-2xl p-6 sm:p-8 text-center">
              ...
              <div className="flex flex-col gap-2.5 sm:gap-3 w-full">
                <button onClick={() => { setIsLockedModalOpen(false); handleOpenPaymentModal(); }} className="w-full py-3 sm:py-3.5 rounded-xl font-bold text-white text-xs sm:text-sm bg-indigo-600 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/25 flex items-center justify-center gap-2">
                  <ShoppingCart size={16} className="shrink-0" /> {isFree ? 'Klaim Gratis Sekarang' : 'Beli Produk Sekarang'}
                </button>
      ...
      <AnimatePresence>
        {isPaymentModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 w-full h-full">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsPaymentModalOpen(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm w-full h-full" />
            <motion.div initial={{ opacity: 0, y: 100, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 100, scale: 0.95 }} className="relative bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl w-full md:w-[600px] max-h-[90vh] md:max-h-[85vh] flex flex-col overflow-hidden">
              <div className="p-6 md:p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 w-full shrink-0">
                <div className="min-w-0">
                  ...
                  <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest truncate w-full">Pilih Channel Favorit Anda</p>
                </div>
              ...
              <div className="p-6 md:p-8 overflow-y-auto flex-1 custom-scrollbar bg-slate-50 w-full">
                {isLoadingChannels ? (
                      ...
                      <div key={groupName} className="w-full">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                          {channels.map((channel: any) => (
                            <button key={channel.code} onClick={() => setSelectedChannel(channel.code)} className={`flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all w-full min-w-0 ${selectedChannel === channel.code ? 'border-indigo-500 bg-indigo-50/50 shadow-md' : 'border-white bg-white hover:border-indigo-200 shadow-sm'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                <p className={`text-sm font-bold truncate w-full ${selectedChannel === channel.code ? 'text-indigo-700' : 'text-slate-700'}`}>
                                  {channel.name}
                                </p>
                              </div>
                            </button>
              ...
              <div className="p-6 md:p-8 bg-white border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 w-full shrink-0">
                <div className="text-center sm:text-left w-full sm:w-auto min-w-0">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1 truncate w-full">Total Tagihan</p>
                  <p className="text-2xl font-black text-slate-900 leading-none truncate w-full">{formatRupiah(product.price)}</p>
                </div>
                <button onClick={() => handleProcessCheckout()} disabled={!selectedChannel || btnLoading || !!channelError} className="w-full sm:w-auto px-8 py-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-[1.25rem] font-black transition-all flex items-center justify-center gap-2 active:scale-95 shrink-0">
                  {btnLoading ? <><Loader2 size={18} className="animate-spin shrink-0" /> Memproses...</> : <>Bayar Sekarang</>}
                </button>
              </div>
            </motion.div>
          </div>
