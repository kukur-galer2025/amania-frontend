...
import {
  ...
  ShoppingCart, Loader2, Layers, UserCircle, Eye, Banknote, Download
} from 'lucide-react';
  ...
  return (
    <div className="font-sans bg-[#F8FAFC] dark:bg-[#0B1120] min-h-screen text-slate-900 dark:text-slate-100 w-full flex flex-col relative overflow-x-clip">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20">
         <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-700/30 p-3 md:p-6 flex overflow-x-auto custom-scrollbar md:flex-wrap md:justify-between items-center gap-4 md:gap-6 snap-x snap-mandatory">
      ...
      <div id="katalog-section" className="sticky top-[60px] md:top-[64px] z-[40] bg-white/95 dark:bg-[#0f172a]/95 border-y border-slate-200 dark:border-slate-700/50 w-full py-3 md:py-4 mt-6 md:mt-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 md:gap-4">
            ...
            <div className="flex items-center w-full sm:w-auto shrink-0">
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl bg-slate-900 hover:bg-indigo-600 text-white font-bold text-xs md:text-sm transition-all active:scale-95"
              >
            ...
            {hasFilters && (
              <motion.div initial={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: 'auto', marginTop: 12 }} exit={{ opacity: 0, height: 0, marginTop: 0 }} className="flex flex-wrap items-center gap-2 overflow-hidden border-t border-slate-100 dark:border-slate-700/30 pt-3 md:pt-4">
                ...
                {selectedCategory !== 'Semua' && (
                  <span className="inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-md md:rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 text-[10px] md:text-xs font-bold border border-indigo-200 shrink-0">
                ...
                {priceFilter !== 'all' && (
                  <span className={`inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-md md:rounded-lg text-[10px] md:text-xs font-bold border shrink-0 ${priceFilter === 'owned' ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 border-indigo-200' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 border-emerald-200'}`}>
                ...
                {sort !== 'newest' && (
                  <span className="inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-md md:rounded-lg bg-slate-100 dark:bg-slate-800/300 text-slate-700 dark:text-slate-300 text-[10px] md:text-xs font-bold border border-slate-200 shrink-0">
                ...
                <button onClick={clearAll} className="inline-flex items-center gap-1 md:gap-1.5 ml-1 md:ml-2 text-[10px] md:text-[11px] font-bold text-rose-500 hover:text-white hover:bg-rose-50 dark:bg-rose-500 px-2.5 md:px-3 py-1 md:py-1.5 rounded-md md:rounded-lg transition-all shrink-0">
              ...
              {filtered.map((p, idx) => {
                ...
                return (
                  <div key={p.id} className="group h-full animate-fade-in" style={{ animationDelay: `${Math.min(idx * 0.04, 0.25)}s`, animationFillMode: 'both' }}>
                    <div
                      className="relative h-full flex flex-col bg-white dark:bg-[#111827] rounded-xl border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-slate-600 cursor-pointer overflow-hidden"
                      onClick={() => router.push(`/e-products/${p.slug}`)}
                    >
                      {/* COVER IMAGE */}
                      <div className="relative aspect-[4/3] overflow-clip bg-slate-100 dark:bg-slate-800 rounded-t-2xl">
                        {p.cover_image ? (
                          <img loading="lazy" src={`${storageUrl}/${p.cover_image}`} alt={p.title} className="w-full h-full object-cover rounded-t-2xl" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = 'https://placehold.co/600x400/1e293b/334155?text=Amania'; }} />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-t-2xl">
                        ...
                        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
                          {owned ? (
                            <span className="bg-indigo-600/90 text-white text-[9px] font-black uppercase px-2.5 py-1 rounded-lg flex items-center gap-1"><CheckCircle2 size={11}/> Dimiliki</span>
                          ) : free ? (
                            <span className="bg-emerald-600/90 text-white text-[9px] font-black uppercase px-2.5 py-1 rounded-lg flex items-center gap-1"><Gift size={11}/> Gratis</span>
                          ) : (
                            <span className="bg-orange-600 text-white text-[9px] font-black uppercase px-2.5 py-1 rounded-md flex items-center gap-1"><Crown size={11}/> Premium</span>
                          )}
                        </div>

                        {/* Rating badge top right */}
                        {avg > 0 ? (
                          <div className="absolute top-3 right-3 z-10 flex items-center gap-1 bg-white/90 dark:bg-slate-900/90 px-2 py-1 rounded-lg border border-slate-200/50 dark:border-slate-700/50">
                            <Star size={12} className="fill-amber-400 text-amber-400" />
                            <span className="text-[11px] font-black text-amber-700 dark:text-amber-400">{avg.toFixed(1)}</span>
                          </div>
                        ) : (
                          <div className="absolute top-3 right-3 z-10 flex items-center gap-1 bg-white/95 dark:bg-slate-900/90 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="flex flex-col flex-1 p-4 sm:p-5">
                        ...
                        <div className="border-t border-slate-100 dark:border-slate-800 pt-3 space-y-3">
                          ...
                          <div className="flex items-center gap-2 w-full">
                            {owned ? (
                              <button onClick={(e) => { e.stopPropagation(); router.push('/my-e-products'); }} className="w-full py-2.5 rounded-xl font-black text-white text-[11px] bg-indigo-600 hover:bg-indigo-700 flex items-center justify-center gap-1.5 active:scale-95">
                                <Layers size={14}/> Buka Koleksi
                              </button>
                            ) : (
                              <>
                                {!free && (
                                  <button onClick={(e) => handleAddToCart(e, p.id)} disabled={isAdding} className="w-10 h-10 shrink-0 rounded-xl font-black text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-100 dark:hover:bg-amber-900/40 border border-amber-200 dark:border-amber-900/50 flex items-center justify-center active:scale-95 transition-all disabled:opacity-50" title="Tambah Keranjang">
                                    {isAdding ? <Loader2 size={15} className="animate-spin" /> : <ShoppingCart size={16} />}
                                  </button>
                                )}
                                <button onClick={(e) => handleOpenPaymentModal(e, p)} className="flex-1 py-2.5 rounded-xl font-black text-white text-[11px] bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700 flex items-center justify-center gap-1.5 active:scale-95 transition-all relative overflow-hidden group/btn border border-emerald-500 dark:border-emerald-500">
        ...
        {isFilterModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-[1.5rem] sm:rounded-[2rem] shadow-lg overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] z-10"
            >
              ...
              <div className="p-5 md:p-6 border-t border-slate-100 dark:border-slate-850 bg-white dark:bg-slate-900 flex items-center gap-3 shrink-0">
                ...
                <button 
                  onClick={() => setIsFilterModalOpen(false)} 
                  className="flex-[2] py-3 md:py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs md:text-sm active:scale-95 flex justify-center items-center gap-2"
                >
                  Terapkan Filter
                </button>
              </div>
