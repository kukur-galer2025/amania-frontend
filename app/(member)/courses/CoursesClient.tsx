  ...
  return (
    <div className="min-h-screen pb-16 font-sans selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <div className="relative overflow-hidden rounded-3xl mb-8 md:mb-12">
        ...
        {/* Ambient Glow Orbs */}
        <div className="absolute top-[-30%] right-[-10%] w-[500px] h-[500px] rounded-full bg-emerald-500/20 blur-[120px]" />
        ...
        <div className="relative px-6 md:px-12 py-12 md:py-16 lg:py-20">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
            <div className="flex-1 max-w-2xl">
              ...
              >
                <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3.5 py-2 rounded-xl border border-white/10">
            ...
            >
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/10 backdrop-blur-xl rounded-2xl py-4 pl-16 pr-6 text-sm font-semibold text-white placeholder-white/40 focus:bg-white/15 focus:ring-2 focus:ring-emerald-500/30 border border-white/10 outline-none shadow-2xl shadow-black/20 transition-all"
                />
              </div>
            </motion.div>
          </div>
      ...
      <div className="flex flex-col gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all border whitespace-nowrap shrink-0 ${
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
            {[
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-[11px] font-bold transition-all whitespace-nowrap shrink-0 ${
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-5 text-center">
          ...
          <h3 className="text-xl font-black text-slate-800">Kursus Belum Tersedia</h3>
          <p className="text-sm text-slate-500 max-w-md leading-relaxed">Kursus yang sesuai dengan pencarian Anda belum tersedia. Coba ubah filter atau cek kembali nanti.</p>
          {(searchQuery || selectedCategory !== 'all' || selectedLevel !== 'all') && (
            <button onClick={() => { setSearchQuery(''); setSelectedCategory('all'); setSelectedLevel('all'); }} className="px-6 py-3 bg-slate-100 text-slate-600 hover:bg-slate-200 text-sm font-bold rounded-xl transition-all">
              Reset Semua Filter
            </button>
          )}
        </div>
            ...
            {filteredCourses.map((course, idx) => (
              ...
              >
                <Link href={`/courses/${course.slug}`} className="group block h-full">
                  <div className="bg-white rounded-2xl md:rounded-3xl border border-slate-200/60 overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_60px_rgba(16,185,129,0.12)] hover:border-emerald-200/60 transition-all duration-500 hover:-translate-y-1.5 h-full flex flex-col">
                    ...
                    <div className="relative aspect-video overflow-hidden bg-slate-100">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
                        <div className="w-14 h-14 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center shadow-2xl group-hover:scale-100 scale-75 transition-transform duration-500">
                      ...
                      <div className="absolute top-3 left-3 right-3 flex items-start justify-between">
                        {/* Level badge */}
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider backdrop-blur-md shadow-lg ${getLevelColor(course.level)}`}>
                          {getLevelIcon(course.level)} {getLevelLabel(course.level)}
                        </span>
                        
                        {/* Price badge */}
                        {course.price === 0 ? (
                          <span className="px-3 py-1.5 rounded-lg text-xs font-black shadow-lg backdrop-blur-md bg-emerald-500/90 text-white">
                            GRATIS
                          </span>
                        ) : (
                          <div className="flex flex-col items-end gap-0.5">
                            <div className="flex items-center gap-1.5 bg-white/90 backdrop-blur-md px-2.5 py-1 rounded-lg shadow-lg">
                              <span className="text-[9px] font-bold text-slate-400 line-through">{formatRupiah(course.price * 5)}</span>
                              <span className="text-[8px] font-black text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">80%</span>
                            </div>
                            <span className="px-3 py-1.5 rounded-lg text-xs font-black shadow-lg backdrop-blur-md bg-white/90 text-slate-900">
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-1.5 mb-3">
                        ...
                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.15em]">{course.category?.name || 'Umum'}</span>
                      </div>

                      {/* Title */}
                      <h3 className="text-[15px] md:text-base font-bold text-slate-900 leading-snug mb-3 line-clamp-2 group-hover:text-emerald-700 transition-colors duration-300">
                        {course.title}
                      </h3>

                      {/* Instructor */}
                      <div className="flex items-center gap-2.5 mb-4">
                        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-slate-100 to-slate-50 border border-slate-200 flex items-center justify-center shrink-0">
                          <User size={13} className="text-slate-400" />
                        </div>
                        <span className="text-xs font-semibold text-slate-500">{course.instructor?.name || 'Amania Team'}</span>
                      </div>
                    ...
                    <div className="px-5 pb-5">
                      <div className="flex items-center justify-between bg-slate-50 group-hover:bg-emerald-50 rounded-xl px-4 py-3 border border-slate-100 group-hover:border-emerald-100 transition-all duration-300">
                        <span className="text-xs font-bold text-slate-500 group-hover:text-emerald-700 transition-colors">Lihat Detail</span>
                        <ArrowRight size={14} className="text-slate-400 group-hover:text-emerald-600 group-hover:translate-x-1 transition-all duration-300" />
                      </div>
                    </div>
                  </div>
                </Link>
