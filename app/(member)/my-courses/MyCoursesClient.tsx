...
export default function MyCoursesClient() {
  ...
  return (
    <div className="min-h-screen">
      {/* Hero */}
      <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-800 rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-10 mb-8 shadow-xl">
        ...
        <div className="relative z-10">
          <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-emerald-200 uppercase tracking-[0.2em] bg-white/10 px-3 py-1.5 rounded-full backdrop-blur-sm mb-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div className="relative flex-1 w-full sm:max-w-xs">
          ...
          <input type="text" placeholder="Cari kursus..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 outline-none" />
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {[
            ...
            <button key={f.value} onClick={() => setStatusFilter(f.value)}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border whitespace-nowrap shrink-0 ${statusFilter === f.value
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-sm font-bold text-slate-400">Memuat kursus...</p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          {courses.length === 0 && (
            <Link href="/courses" className="mt-4 px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200 flex items-center gap-2">
            ...
            return (
              ...
              >
                <Link href={`/learn/${course.slug}`} className="group block">
                  <div className="bg-white rounded-2xl border border-slate-200/60 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:border-slate-200 transition-all duration-500 hover:-translate-y-1">
                    {/* Thumbnail */}
                    <div className="relative aspect-video overflow-hidden bg-slate-100">
                      ...
                      <div className="absolute top-3 right-3">
                        {status === 'completed' ? (
                          <span className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-emerald-500 text-white shadow-lg flex items-center gap-1">
                            <CheckCircle2 size={12} /> Selesai
                          </span>
                        ) : status === 'in_progress' ? (
                          <span className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-amber-500 text-white shadow-lg">
                            {progress}%
                          </span>
                        ) : (
                          <span className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-slate-600 text-white shadow-lg">
                            Baru
                          </span>
                        )}
                      </div>
                    ...
                    {/* Content */}
                    <div className="p-5">
                      <h3 className="text-base font-bold text-slate-900 leading-snug mb-3 line-clamp-2 group-hover:text-emerald-700 transition-colors">
                        {course.title}
                      </h3>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1 text-[11px] font-bold text-slate-500">
                            <BookOpen size={13} className="text-slate-400" /> {course.sections_count || 0} Bab
                          </span>
                          <span className="flex items-center gap-1 text-[11px] font-bold text-slate-500">
                            <Video size={13} className="text-slate-400" /> {course.total_lessons || 0} Lesson
                          </span>
                        </div>
                        {/* Rating button */}
                        <button
        ...
        {ratingModal && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-x-4 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 top-1/2 -translate-y-1/2 w-auto sm:w-full sm:max-w-md bg-white rounded-2xl shadow-2xl z-50 overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <button onClick={() => setRatingModal(null)} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
              ...
              <div className="px-6 py-5">
                {/* Course title */}
                <p className="text-sm font-bold text-slate-700 mb-5 line-clamp-2">{ratingModal.title}</p>
                ...
                <div className="flex items-center justify-center gap-2 mb-5">
                  {[1, 2, 3, 4, 5].map(star => (
                    ...
                    >
                      <Star
                        size={32}
                        className={`transition-colors ${star <= (hoverRating || reviewRating) ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`}
                      />
                    </button>
                  ))}
                </div>
                ...
                <div className="mb-1">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium resize-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400 outline-none"
                  />
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                <button onClick={() => setRatingModal(null)} className="px-4 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors">
                  Batal
                </button>
                <button
                  onClick={handleSubmitReview}
                  disabled={submittingReview || reviewRating === 0}
                  className="flex items-center gap-2 px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-xl transition-all disabled:opacity-50 active:scale-95 shadow-lg shadow-amber-500/20"
                >
