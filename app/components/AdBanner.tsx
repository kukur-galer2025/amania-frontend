...
export default function AdBanner({ placement, className = '' }: AdBannerProps) {
  ...
  return (
    <section className={`px-4 sm:px-6 lg:px-8 py-4 max-w-7xl mx-auto w-full relative z-10 ${className}`}>
      ...
      >
        {/* Image Container — fixed aspect ratio for consistency across all slides */}
        <div className="relative w-full aspect-[16/5]">
          ...
          <div className="absolute top-2 left-2 md:top-5 md:left-5 z-20">
            <div className="flex items-center gap-1 md:gap-1.5 px-2 md:px-3 py-1 md:py-1.5 bg-slate-900/80 rounded-full border border-amber-500/30">
              <Sparkles size={10} className="text-amber-400 md:w-3 md:h-3" />
              <span className="text-[8px] md:text-[10px] font-black text-amber-400 tracking-widest uppercase">Sorotan Eksklusif</span>
            </div>
          </div>
          ...
          {ads.length > 1 && (
            <>
              <button
                onClick={goPrev}
                className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-20 w-8 h-8 md:w-12 md:h-12 rounded-full bg-slate-900/60 hover:bg-slate-900/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 border border-white/20"
              >
                <ChevronLeft size={16} className="md:w-5 md:h-5" />
              </button>
              <button
                onClick={goNext}
                className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-20 w-8 h-8 md:w-12 md:h-12 rounded-full bg-slate-900/60 hover:bg-slate-900/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 border border-white/20"
              >
          ...
          {/* Dot Indicators */}
          {ads.length > 1 && (
            <div className="absolute bottom-2 md:bottom-4 left-0 right-0 flex justify-center gap-1 md:gap-2 z-20">
              {ads.map((_, idx) => (
                <button
                  key={idx}
                  onClick={() => goTo(idx)}
                  className={`h-1 md:h-2 rounded-full transition-all duration-300 ${
                    idx === currentIndex
                      ? 'bg-amber-400 w-5 md:w-10'
                      : 'bg-white/40 hover:bg-white/80 w-1.5 md:w-3'
                  }`}
                />
              ))}
            </div>
