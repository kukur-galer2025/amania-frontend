...
import { ArrowLeft, Save, Loader2, BookOpen, Layers, CheckCircle2, LayoutGrid, Tag } from 'lucide-react';
...
function CreateQuestionContent() {
  ...
  // Variabel disesuaikan dengan database baru
  const [formData, setFormData] = useState({
    skd_tryout_id: skdTryoutIdFromUrl || '',
    main_category: 'twk',
  ...
  useEffect(() => {
    if (!skdTryoutIdFromUrl) return;

    const fetchData = async () => {
      try {
        const [resPkg, resSub, resQ] = await Promise.all([
        ...
        if (jsonQ.success) setExistingQuestions(jsonQ.data);
      } catch (error) {
  ...
  }, [formData.main_category, correctOption]);
  ...
  const validateForm = () => {
    if (!formData.skd_tryout_id) return "Master Tryout belum dipilih dari URL.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch('/admin/tryout/skd/questions', {
        method: 'POST',
      ...
      if (res.ok && json.success) {
        const currentNumber = existingQuestions.length + 1;
        toast.success(`Soal Nomor ${currentNumber} berhasil ditambahkan!`, { 
          duration: 4000,
          icon: '✅',
        ...
        const resQ = await apiFetch(`/admin/tryout/skd/questions?skd_tryout_id=${skdTryoutIdFromUrl}`);
        const jsonQ = await resQ.json();
        if (jsonQ.success) setExistingQuestions(jsonQ.data);
        ...
        toast.error(json.message || 'Gagal menyimpan soal ke server');
      }
    } catch (error) {
  ...
  if (!skdTryoutIdFromUrl) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="text-sm font-bold text-slate-500">ID Tryout tidak ditemukan. Silakan kembali ke halaman kelola dan pilih tryout yang benar.</p>
  ...
  if (fetchingData) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Membuka Workspace Ujian...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900">Buat Soal Baru</h1>
          <p className="text-slate-500 text-sm font-medium">Tambahkan soal untuk <span className="font-bold text-indigo-600">{packageInfo?.title}</span></p>
        </div>
      </div>
          ...
          <form id="questionForm" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-[1.5rem] border border-slate-200 shadow-sm p-6 sm:p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Kolom Kiri: Tryout & Kategori Tryout (Disabled) */}
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-2">Tryout <span className="text-rose-500">*</span></label>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 cursor-not-allowed opacity-80">
                      ...
                      {/* Menampilkan nama kategori dari database dengan fallback jika belum ada relasinya */}
                ...
                {/* Kolom Kanan: Kategori Induk Soal & Sub Kategori Materi */}
                <div className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-2">Jenis Soal (Induk) <span className="text-rose-500">*</span></label>
                    <select 
            ...
            <div className="flex justify-end gap-4 p-4">
              <button type="submit" form="questionForm" disabled={loading} className="w-full sm:w-auto px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl active:scale-95 disabled:opacity-70 text-sm">
                {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} Simpan & Lanjut Nomor {existingQuestions.length + 1}
              </button>
            </div>
          </form>
        </div>
            ...
            <div className="grid grid-cols-5 gap-2 max-h-[50vh] overflow-y-auto custom-scrollbar pr-2">
              {existingQuestions.map((q, index) => (
                <Link 
                  ...
                  className="w-full aspect-square flex items-center justify-center rounded-xl text-xs font-bold border transition-all border-slate-200 bg-white text-slate-600 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-600"
                  title={`Edit Soal #${index + 1}`}
                >
                  {index + 1}
                </Link>
              ))}
              
              <div className="w-full aspect-square flex items-center justify-center rounded-xl text-xs font-black border-2 border-indigo-600 bg-indigo-600 text-white shadow-md shadow-indigo-200" title="Soal Baru">
