...
import { ArrowLeft, Save, Loader2, Target, Clock, Calendar, Flame, Eye, Layers } from 'lucide-react';
import toast from 'react-hot-toast';
import { apiFetch } from '@/app/utils/api';
import RichTextEditor from '@/app/components/RichTextEditor'; 

export default function CreateSkdTryoutPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [categories, setCategories] = useState<any[]>([]); // State untuk Kategori dari DB
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (json.success) {
          ...
          // Set default value ke kategori pertama jika ada
          if (json.data.length > 0) {
            setFormData(prev => ({ ...prev, skd_tryout_category_id: json.data[0].id.toString() }));
          }
        }
      } catch (err) {
        toast.error('Gagal mengambil daftar kategori tryout.');
      }
    };
    fetchCategories();
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900">Buat Tryout SKD Baru</h1>
          <p className="text-slate-500 text-sm font-medium">Lengkapi form di bawah untuk membuat master sesi ujian.</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="bg-white rounded-[1.5rem] border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="title" value={formData.title} onChange={handleChange} required placeholder="Contoh: Tryout SKD Premium 01" className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-11 pr-4 text-sm font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select 
                      name="skd_tryout_category_id" // 🔥 Menggunakan properti database baru
                    ...
                    >
                      {categories.length === 0 && <option value="" disabled>Memuat...</option>}
                      {categories.map((cat) => (
        ...
        <div className="px-6 sm:px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 relative z-20">
          <Link href="/admin/tryouts/skd/manage" className="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-100 text-slate-600 rounded-xl font-bold transition-all text-sm">Batal</Link>
          <button type="submit" disabled={loading} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-70 text-sm">
            {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} Simpan Tryout Baru
          </button>
        </div>
      </form>
    </div>
